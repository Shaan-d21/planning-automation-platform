  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="job-dialog" role="dialog" aria-modal="true" aria-labelledby="job-dialog-title">
    ...
    <div className="job-dialog-body">
      ...
      {tab === "overview" && <>
        ...
        {job.record_statistics && <div className="job-overview-results" aria-label="Oracle load summary"><span><small>Records read</small><strong>{job.record_statistics.records_read.toLocaleString()}</strong></span><span><small>{job.record_statistics.source.startsWith("ORACLE_DATA_INTEGRATION") ? "Processed / loaded" : "Records processed"}</small><strong>{job.record_statistics.records_processed.toLocaleString()}</strong></span><span className={job.record_statistics.records_rejected ? "is-warning" : ""}><small>Records rejected</small><strong>{job.record_statistics.records_rejected.toLocaleString()}</strong></span><button onClick={() => setTab("results")}>View breakdown <Icon name="arrow" /></button></div>}
      ...
      {tab === "results" && (job.record_statistics ? <OracleLoadStatistics statistics={job.record_statistics} /> : <EvidenceEmpty title="No Oracle load counters" message="This operation or Oracle environment did not expose one unambiguous set of records read, processed, and rejected through supported job details or Data Integration logs." />)}
...
function JobFiles({ artifacts }: { artifacts: JobActivityDetail["artifacts"] }) {
  ...
  return <section className="job-file-list"><div className="evidence-section-heading"><div><span className="eyebrow">Execution artifacts</span><h3>Files from Oracle</h3><p>Oracle job logs, error archives, and extracted rejection files are retained with this execution.</p></div></div>{artifacts.map((artifact) => <a href={artifact.download_url} className="job-file" key={artifact.artifact_id}><span><Icon name="reports" /></span><div><strong>{artifact.name}</strong><small>{friendlyStatus(artifact.kind)} · {formatBytes(artifact.size_bytes)}</small></div><em>Download</em><Icon name="arrow" /></a>)}</section>;
}
