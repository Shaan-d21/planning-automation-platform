  ...
  return <section className="access-workspace">
    <header className="page-intro access-intro">
      ...
      <button className="button button--primary" onClick={() => setEditor({ ...EMPTY_EDITOR })}>
        <Icon name="users" /> Add local recovery user
      </button>
    </header>
    ...
    <section className="panel identity-access-explainer" aria-labelledby="identity-access-title">
      <div>
        ...
        <h2 id="identity-access-title">Oracle first, local recovery second</h2>
        <p>People normally sign in with their Oracle EPM account. Existing local platform accounts remain available for recovery and administration.</p>
      </div>
      <div className="identity-access-modes">
        <article className="is-primary"><span><Icon name="users" /></span><div><strong>Primary · Oracle EPM account</strong><small>{data.identity_sync.sso_enabled ? "Oracle SSO verifies the identity; the platform never receives the Oracle password." : "Credentials are validated against Oracle EPM for this sign-in and are never stored."}</small></div><em>{data.identity_sync.sso_enabled || data.identity_sync.oracle_password_login_enabled ? "Enabled" : "Setup required"}</em></article>
        <article><span><Icon name="settings" /></span><div><strong>Secondary · Local recovery account</strong><small>Existing local usernames and passwords continue to work independently of Oracle.</small></div><em>Available</em></article>
      </div>
    </section>
    ...
    <section className="panel identity-sync-panel">
      ...
      <div className="identity-setup-steps" aria-label="Oracle account setup steps">
        ...
        <article className={mappedEntitlements > 0 ? "is-current" : ""}><b>3</b><span><strong>Optional pre-provision review</strong><small>Review linked profiles now, or let them be created at first Oracle sign-in</small></span></article>
      </div>
      <p className="identity-sync-footnote"><strong>Oracle sign-in uses just-in-time provisioning.</strong> After mappings are saved, an eligible person’s linked profile is created or refreshed at sign-in; their Oracle password is never retained. Administrators can optionally pre-provision profiles from the review below.</p>
      {identityMappings && <details className="identity-mapping-disclosure" open={mappedEntitlements === 0}>
        ...
        <div className="identity-mapping-content">
          ...
          {visibleEntitlements.length ? <div className="identity-mapping-list">{visibleEntitlements.map((entitlement) => <article key={entitlement.entitlement_id}>
            <div><span className="entitlement-type">{entitlement.entitlement_type.replaceAll("_", " ")}</span><strong>{entitlement.display_name}</strong><small>{entitlement.assigned_identity_count ? `${entitlement.assigned_identity_count} Oracle ${entitlement.assigned_identity_count === 1 ? "person has" : "people have"} this access` : "Available Oracle role; no synchronized assignments currently visible"}</small>{entitlement.mapped_role && entitlement.assigned_identity_count > 0 && <p className="identity-mapping-impact">All {entitlement.assigned_identity_count} will derive <b>{roleByCode.get(entitlement.mapped_role)?.name ?? entitlement.mapped_role}</b> unless a higher mapped role also applies.</p>}</div>
            <label><span>Platform access</span><select aria-label={`Platform role for ${entitlement.display_name}`} disabled={identitySyncBusy} value={entitlement.mapped_role ?? ""} onChange={(event) => void onSetIdentityMapping(entitlement.entitlement_id, (event.target.value || null) as PlatformRoleCode | null)}><option value="">Do not grant access from this entitlement</option>{data.roles.map((role) => <option key={role.code} value={role.code}>{role.name}</option>)}</select></label>
          ...
          <div className="identity-provision-action"><div><strong>Step 3 · Optional pre-provision review</strong><p>Review every currently synchronized person affected by the saved mappings. You can approve them now, or allow each eligible profile to be created automatically at first Oracle sign-in.</p></div><button className="button button--primary" disabled={identitySyncBusy || mappedEntitlements === 0} onClick={() => void onPreviewProvisioning()}>{identitySyncBusy ? "Preparing…" : "Preview linked profiles"}</button></div>
        </div>
      </details>}
    </section>
    ...
    <section className="panel access-user-panel">
      <div className="panel-heading"><div><span className="eyebrow">People</span><h2>Accounts and linked profiles</h2><p>Oracle-linked profiles are governed by entitlement mappings. Local recovery accounts are managed here; inactive records remain for audit history.</p></div><span className="result-count">{users.length} shown</span></div>
      ...
      {users.length ? <div className="access-table-wrap"><table className="access-table"><thead><tr><th>User</th><th>Source</th><th>Role</th><th>Status</th><th>Last sign in</th><th><span className="sr-only">Actions</span></th></tr></thead><tbody>
        {users.map((user) => <tr key={user.user_id}>
          ...
          <td><span className={`account-source account-source--${user.authentication_source.toLowerCase().replaceAll("_", "-")}`}>{accountSourceLabel(user.authentication_source)}</span></td>
          ...
          <td>{user.authentication_source === "LOCAL_RECOVERY" ? <div className="access-row-actions"><button className="button button--quiet" onClick={() => editUser(user)}>Edit</button><button className="icon-button" aria-label={`Reset password for ${user.display_name}`} onClick={() => { setPasswordUser(user); setNewPassword(""); }}><Icon name="settings" /></button></div> : <span className="managed-account-label"><Icon name="check" /> Mapping managed</span>}</td>
        </tr>)}
    ...
    {editor && <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setEditor(null); }}><section className="access-dialog" role="dialog" aria-modal="true" aria-labelledby="access-dialog-title">
      <header><div><span className="eyebrow">{editor.userId === null ? "New recovery account" : "Edit recovery account"}</span><h2 id="access-dialog-title">{editor.userId === null ? "Add a local recovery user" : `Manage ${editor.displayName}`}</h2><p>Assign one primary platform role. Oracle-linked people are created from the synchronized mappings instead.</p></div><button aria-label="Close" onClick={() => setEditor(null)}><Icon name="close" /></button></header>
      <form onSubmit={save}>
...
function IdentitySyncPreviewDialog({ preview, busy, onCancel, onApply }: { preview: IdentitySyncPreview; busy: boolean; onCancel: () => void; onApply: () => Promise<void> }) {
  ...
  return <div className="modal-backdrop identity-sync-backdrop" role="presentation" onMouseDown={(event) => { if (!busy && event.target === event.currentTarget) onCancel(); }}>
    <section className="access-dialog identity-sync-dialog" role="dialog" aria-modal="true" aria-labelledby="identity-sync-title">
      ...
      <div className="identity-sync-dialog__body">
    ...
    <section className="access-dialog identity-sync-dialog" role="dialog" aria-modal="true" aria-labelledby="identity-provision-title">
      ...
      <div className="identity-sync-dialog__body">
        ...
        <div className="identity-preview-table-wrap">
          ...
          </tr>)}</tbody></table> : <div className="work-empty work-empty--compact"><span><Icon name="check" /></span><h3>Linked profiles already match</h3><p>No provisioning changes are required.</p></div>}
        </div>
      </div>
      <footer><button type="button" className="button button--quiet" disabled={busy} onClick={onCancel}>Cancel</button><button type="button" className="button button--primary" disabled={busy || actionable === 0} onClick={() => void onApply()}>{busy ? "Verifying state…" : `Approve ${actionable} profile ${actionable === 1 ? "change" : "changes"}`}</button></footer>
    </section>
  </div>;
}
...
function accountSourceLabel(source: PlatformUser["authentication_source"]) {
  if (source === "ORACLE_LINKED") return "Oracle linked";
  if (source === "ORACLE_AND_LOCAL") return "Oracle + legacy local";
